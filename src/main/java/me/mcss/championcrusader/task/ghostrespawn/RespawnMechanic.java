...
public class RespawnMechanic extends BukkitRunnable {
    ...
    // Main Constructor
    public RespawnMechanic(ChampionCrusader plugin, Player player, Player playerKiller) {
        this.plugin = plugin;
        this.player = player;
    ...
    public RespawnMechanic(ChampionCrusader plugin, Player player) {
        this.plugin = plugin;
        this.player = player;
    ...
    public void run() {
        ...
        BukkitTask countFive = new Countdown(player, ChatColor.RED + "5", 0.2f).runTaskLater(this.plugin, 100L);
        BukkitTask countFour = new Countdown(player, ChatColor.RED + "4", 0.4f).runTaskLater(this.plugin, 120);
        BukkitTask countThree = new Countdown(player, ChatColor.YELLOW + "3", 0.6f).runTaskLater(this.plugin, 140L);
        BukkitTask countTwo = new Countdown(player, ChatColor.YELLOW + "2", 0.8f).runTaskLater(this.plugin, 160L);
        BukkitTask countOne = new Countdown(player, ChatColor.GREEN + "1", 1.0f).runTaskLater(this.plugin, 180L);

        // Respawn task -> see AfterCountdown for info
        BukkitTask afterCountdown = new AfterCountdown(player, plugin).runTaskLater(this.plugin, 200L);
    }
}
